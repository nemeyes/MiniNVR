...
unsigned BitVector::getBits(unsigned numBits) 
{
    ...
    shiftBits(tmpBuf, 0, /* to */fBaseBytePtr, fBaseBitOffset + fCurBitIndex, /* from */numBits - overflowingBits /* num bits */);
    fCurBitIndex += numBits - overflowingBits;
...
unsigned removeH264or5EmulationBytes(unsigned char * to, unsigned toMaxSize, unsigned char const * from, unsigned fromSize) 
{
    unsigned toSize = 0;
    unsigned i = 0;
...
void removeEmulationBytes(unsigned char * dst, int& dst_size, unsigned char * src, int src_size)
{
    //unsigned char const * nalUnitOrig = fStartOfFrame + fOutputStartCodeSize;
    //unsigned const numBytesInNALunit = fTo - nalUnitOrig;
    dst_size = removeH264or5EmulationBytes(dst, src_size, src, src_size);
}